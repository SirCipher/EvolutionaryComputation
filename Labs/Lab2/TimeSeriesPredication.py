...
def runTesting(best):
    ...
    data = np.genfromtxt('testing.csv', delimiter=",")

    print("Running predictions......")

    for row in data:
        estimate = prediction(best, row)

        totalSquDiff = sum((prediction(best, row) - row[4])**2 for row in data)
        error = totalSquDiff / len(data),

        print("Estimate: %s. Actual: %s. Error: %s" % (estimate, row, error ))
